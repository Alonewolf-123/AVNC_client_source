...
module.exports = function create () {
  ...
  var remoteConfigUrl = 'REMOTE URL'
  if (process.browser) remoteConfigUrl = 'REMOTE CONFIG' + remoteConfigUrl
    ...
    events.onmessage = function onMessage (e) {
      ...
      inflate(row.data, function inflated (err, stringified) {
        ...
        //   if (err) return handleRTCErr(err, cb)
          ...
          console.log('stringified', JSON.parse(stringified.toString()))
          // setTimeout(function () {
            peer.signal(JSON.parse(stringified.toString()))
            console.log('remote signal sent')
            cb(null, peer)
          // }, 3000);
        // })
      })
  ...
  function responsePongToHost (data, room, cb) {
    console.log('client pong response')
  ...
  function onConnect (peer, remote) {
    ...
    if (remote) {
      ...
      window.addEventListener('wheel', mousescrollListener)
    ...
    peer.on('close', function cleanup () {
      ...
      window.removeEventListener('wheel', mousescrollListener)
    ...
    function mousemoveListener (e) {
      if (g_control_enabled) {
        ...
        if (data.clientY > 0) peer.send(buf)
      }
    }
    ...
    function mousescrollListener (e) {
    ...
    function getMouseData (e) {
      ...
      for (i = 0; i < screen_count; i++) {
        if (ui.containers.multimedia.childNodes[i].style.display != "none") {
  ...
  function inflate (data, cb) {
    ...
    zlib.inflate(Buffer.from(data, 'base64'), cb)
  }
